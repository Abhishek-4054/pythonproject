        ...
        stage('Verify Rollout') {
            steps {
                withCredentials([sshUserPrivateKey(credentialsId: 'vm-ssh-credentials', keyFileVariable: 'SSH_KEY', usernameVariable: 'SSH_USER')]) {
                    ...
                    ssh -i "%SSH_KEY%" -o StrictHostKeyChecking=no %SSH_USER%@%VM_HOST% "kubectl rollout status deployment/backend --timeout=2m && kubectl rollout status deployment/frontend --timeout=2m && kubectl get pods"
                    """
                }
            }
        }
